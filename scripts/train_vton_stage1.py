...
def main() -> None:
    ...
    garment_encoder = GarmentEncoder(checkpoints.garment_backbone).to(args.device)
    ...
    state = trainer.train(dataloader)
    logger.info("finished at step %d (%d samples)", state.step, state.samples_seen)
